...
/* SCRIPT AJUSTADO DAQUI PRA BAIXO*/
...
async function montarTelaConfirmarPresenca(chamadaId) {
  const raiz = $('#tela-confirmar-presenca');
  raiz.innerHTML = '<div class="confirmacao-carregando"><span class="girador"></span><p>Carregando dados da chamada...</p></div>';

  try {
    // 1. Buscar chamada no banco PostgreSQL via API/Supabase
    const chamada = await apiBuscarChamadaPorId(chamadaId);
    ...
    const turma = chamada.turmas;
    const disciplina = turma?.disciplinas;

    const estado = { chamada, turma, disciplina, aluno: null, fotoCapturada: null };

    renderPassoIdentificacao(raiz, estado);
  } catch (erro) {
    console.error("Erro ao carregar chamada:", erro);
    raiz.innerHTML = '<p class="confirmacao-erro-caixa">Erro ao conectar com o servidor. Tente recarregar a página.</p>';
  }
}
...
function renderPassoIdentificacao(raiz, estado) {
  ...
  form.addEventListener('submit', async (evento) => {
    ...
    try {
      // Valida o aluno diretamente na turma correspondente no banco
      const aluno = await apiValidarAlunoNaTurma(estado.turma.id, ra, email);
      ...
      exibirErroPasso(erro, 'Erro ao validar dados. Tente novamente.');
      btnSubmit.disabled = false;
      btnSubmit.textContent = 'Continuar';
    }
  });
...
function renderPassoRosto(estado) {
  ...
  btnCapturar.addEventListener('click', async () => {
    ...
    const resultado = await compararRosto(estado.fotoCapturada, estado.aluno.foto_rosto);
...
function renderPassoCodigo(estado) {
  ...
  form.addEventListener('submit', async (evento) => {
    ...
    const btnSubmit = form.querySelector('button[type="submit"]');
    btnSubmit.disabled = true;
    btnSubmit.textContent = 'Salvando presença...';

    try {
      // Re-busca a chamada atualizada para checar o código rotativo em tempo real
      const chamadaAtual = await apiBuscarChamadaPorId(estado.chamada.id);
      ...
      const codigoValido = digitado === chamadaAtual.codigo_atual || digitado === chamadaAtual.codigo_anterior;
      if (!codigoValido) {
        exibirErroPasso(erro, 'Código incorreto. Confira o código que está no telão e tente de novo.');
        btnSubmit.disabled = false;
        btnSubmit.textContent = 'Confirmar presença';
      ...
      // Salva ou atualiza a presença no PostgreSQL
      await apiRegistrarPresenca(estado.chamada.id, estado.turma.id, estado.aluno.id);

      renderSucessoConfirmacao(estado);
    } catch (e) {
      exibirErroPasso(erro, 'Erro ao registrar presença no banco. Tente novamente.');
      btnSubmit.disabled = false;
      btnSubmit.textContent = 'Confirmar presença';
    }
  });
